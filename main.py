...
def salvar_redis(id_pokemon: int, pokemon: Pokemon):
    ...
    if REDIS_AVAILABLE:
        ...
        redis_client.set(
            f"pokemon:{id_pokemon}",
            json.dumps(pokemon.model_dump())
        )
...
def filtar_um_pokemon(
    id_pokemon: int,
    db: Session = Depends(sessao_db)
):

    pokemon = db.query(PokemonDB).filter(
        PokemonDB.id_pokemon == id_pokemon
    ).first()
...
def listar_pokemons(
    ...
    if not pokemons:
        ...
        raise HTTPException(
            status_code=404,
            detail="Nao ha pokemons listados"
        )
        ...
        "pokemons": [
            {
                ...
                "nivel_pokemon": pokemon.nivel_pokemon
            }
...
def adicionar_pokemon(
    ...
    poke_db = db.query(PokemonDB).filter(
        PokemonDB.nome_pokemon == pokemon.nome_pokemon,
        PokemonDB.tipo_pokemon == pokemon.tipo_pokemon,
        PokemonDB.nivel_pokemon == pokemon.nivel_pokemon
    ).first()
    ...
    if poke_db:
        ...
        raise HTTPException(
            status_code=409,
            detail="Pokemon ja cadastrado treinador"
        )
    ...
    return {
        "message": f"O {novo_pokemon.nome_pokemon} foi criado com sucesso"
    }
...
def atualizar_pokemon(
    ...
    return {
        "message": "O nivel do pokemon foi atualizado",
        "pokemon": poke_db.nome_pokemon,
        "nivel_antigo": nivel_antigo,
        "nivel_novo": poke_db.nivel_pokemon
    }
...
def evoluir_pokemon(
    ...
    return {
        "message": f"O pokemon evoluio para {poke_db.nome_pokemon}"
    }
...
def deletar_pokemon(
    ...
    if not poke_db:
        ...
        raise HTTPException(
            status_code=404,
            detail="Erro pokemon nao encontrado"
        )
    ...
    return {
        "message": f"O pokemon {poke_db.nome_pokemon} foi excluido"
    }